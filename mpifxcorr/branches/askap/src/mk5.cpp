...
{ 
  ...
  while (mark5_stream_next_frame(syncteststream)==0) {
    ...
    if (ns2000 > endns2000) {
      ...
      cinfo << startl << "Datastream " << mpiid << " detected jumo in data - saving for next segment" << endl;
      if (tempbuf==0) {
	tempbuf = vectorAlloc_u8(bufferbytes/numdatasegments);
	if (tempbuf==NULL) {
	  cerror << startl << "Could not allocate temporary buffer. Skipping bytes after data jump" << endl;
	} else {
