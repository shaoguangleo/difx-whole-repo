/***************************************************************************
 *   Copyright (C) 2006-2011, 2015 by Adam Deller                          *
 *                                                                         *
...
int main(int argc, char *argv[])
{
	...
	if(numcores < 1)
	{
		cfatal << startl << "Must be invoked with at least " << fxcorr::FIRSTTELESCOPEID + numdatastreams + 1 << " processors (was invoked with " << numprocs << " processors, fxcorr::FIRSTTELESCOPEID is " << fxcorr::FIRSTTELESCOPEID << " and numdatastreams is " << numdatastreams << ") - aborting!" << endl;
		MPI_Barrier(world);
		MPI_Finalize();
		return EXIT_FAILURE;
	}
