...
int writeJob(const VexJob& J, const VexData *V, const CorrParams *P, int overSamp, int verbose, ofstream *of, int nDigit, char ext)
{
	...
	for(vector<string>::const_iterator si = J.scans.begin(); si != J.scans.end(); si++, scan++)
	{
		...
		if(sourceSetup)
		{
			...
			{
				spacecraftSet.insert(S->sourceDefName);
			}
		}
	}
	...
	mode = V->getModeByDefName(configs[0].first);
...
static int sanityCheckConsistency(const VexData *V, const CorrParams *P)
{
	...
	for(unsigned int s = 0; s < V->nSource(); s++)
	{
		...
		if(sourceSetup && sourceSetup->difxName.size() > 0)
		{
			...
			{
				cerr << "Warning: illegal source name (" << sourceSetup->difxName << ") provided in SOURCE section for source " << S->defName << endl;
				nWarn++;
			}
		}
		else
