     ...
     char *argv[];
{
     ...
     struct getCALC_9_1_RA_arg request_args, *p_request;
     struct getCALC_9_1_RA_res *p_result;
     ...
     if(argc != 2)
     {
         fprintf(stderr, "Usage : %s <CALC_9_1_RA_Server host>\n\n", argv[0]);
	 return 0;
     }

     if (!(cl = clnt_create (argv[1], CALC_9_1_RAPROG, CALC_9_1_RAVERS, "tcp")))
     {
     ...
     memset(p_request, 0, sizeof(struct getCALC_9_1_RA_arg));

     p_request->date = 50774;
     p_request->time = 22.0/24.0 + 2.0/(24.0*60.0);
     p_request->struct_code = CALC_9_1_RA_SERVER_STRUCT_CODE_5_0_0;
     ...
     */
     ...
     p_request->struct_code = CALC_9_1_RA_SERVER_STRUCT_CODE_0;
     ...
     p_result = getcalc_9_1_ra_1(p_request, cl);
     ...
     {
         printf("Checking %s version 0x%lX call:\n", argv[1], (long)(CALC_9_1_RA_SERVER_STRUCT_CODE_5_0_0));
         v = system ("date -u \"+\%FT\%T.\%NZ\"");
         if(v == -1)
         {
             fprintf(stderr, "Warning -- system() failed\n");
         ...
         printf ("result: request_id = %ld\n", 
                 p_result->getCALC_9_1_RA_res_u.record.request_id);
         printf ("result: date  = %ld\n", 
                 p_result->getCALC_9_1_RA_res_u.record.date);
         printf ("result: time  = %e\n",
                 p_result->getCALC_9_1_RA_res_u.record.time);
         printf ("result: delay[0] = %16.10e\n", 
                 p_result->getCALC_9_1_RA_res_u.record.delay[0]);
         printf ("result: delay[1]  = %16.10e\n", 
                 p_result->getCALC_9_1_RA_res_u.record.delay[1]);
         printf ("result: dry_atmos[0]  = %e\n", 
                 p_result->getCALC_9_1_RA_res_u.record.dry_atmos[0]);
         printf ("result: wet_atmos[0]  = %e\n", 
                 p_result->getCALC_9_1_RA_res_u.record.wet_atmos[0]);
         printf ("result: elev[0]  = %e\n", 
                 p_result->getCALC_9_1_RA_res_u.record.el[0]*57.296);
         printf ("result: azim[0]  = %e\n", 
                 p_result->getCALC_9_1_RA_res_u.record.az[0]*57.296);
         /**/
         delta_dly = p_result->getCALC_9_1_RA_res_u.record.delay[0];
         delta_dly = fabs(delta_dly - (-2.04212341289221e-02));

         if (delta_dly >= 1.0e-13)
         {
             printf ("ERROR : CALC_9_1_RA_9_1_RA_Server with version 0x%lX input is returning BAD DATA.\n", (long)(CALC_9_1_RA_SERVER_STRUCT_CODE_5_0_0));
     ...
     if(calcserver_version >= CALC_9_1_RA_SERVER_STRUCT_CODE_5_1_0)
     {
         ...
         printf("Checking %s version 0x%lX call:\n", argv[1], (long)(CALC_9_1_RA_SERVER_STRUCT_CODE_5_1_0));
         v = system ("date -u \"+\%FT\%T.\%NZ\"");
         if(v == -1)
         {
             fprintf(stderr, "Warning -- system() failed\n");
         ...
         p_request->struct_code = CALC_9_1_RA_SERVER_STRUCT_CODE_5_1_0;
         p_result = getcalc_9_1_ra_1(p_request, cl);
     ...
     if((mode_failures))
     {
         printf ("ERROR : CALC_9_1_RA_Server is returning BAD DATA.\n");
         printf ("        Restart the CALC_9_1_RA_Server.\n");
         return 1;
     }
