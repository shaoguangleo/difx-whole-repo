...
//
// $Id: calcif3.cpp 6697 2015-06-05 13:26:42Z JamesAnderson $
// $HeadURL: $
...
typedef struct
{
	...
	enum DelayServerHandlerType delayServerHandlerType;
	int nFile;
	int polyOrder;
...
static void usage()
{
	...
	fprintf(stderr, "\n");
	fprintf(stderr, "  --num-CALC-threads <uint>\n");
	fprintf(stderr, "                          Use <uint> threads for separate CALC\n");
	fprintf(stderr, "                          instances to speed up delay calculations\n");
	fprintf(stderr, "                          when using the Pipe handler mode.\n");
	fprintf(stderr, "                          0 means use the environment variable\n");
	fprintf(stderr, "                          DIFX_NUMBERCALCSERVERTHREADS value. [0]\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "\n");
	...
	fprintf(stderr, "  -s       <servername>   Use <servername> as hostname for the DiFX_Delay_Server RPC delay server\n\n");
	...
	for(ds = 0; ds < int64_t(NumDelayServerTypes); ++ds)
	{
		fprintf(stderr, "          %s\n", delayServerTypeNames[ds]);
	}
	fprintf(stderr, "\n");
	fprintf(stderr, "  --server-handler-type <type>    Use <type> as the type of delay server handler to use\n\n");
	fprintf(stderr, "      Allowed values are\n");
	for(ds = 0; ds < int64_t(NumDelayServerHandlerTypes); ++ds)
	{
		fprintf(stderr, "          %s\n", delayServerHandlerTypeNames[ds]);
	}
	fprintf(stderr, "\n");
}
...
static CommandLineOptions *newCommandLineOptions(int argc, char **argv)
{
	...
	opts->delayServerHandlerType = UnknownDelayServerHandler; /* default to value in the .calc file */
	...
	opts->Num_CALC_Threads = 0; /* use default */
	...
	for(i = 1; i < argc; ++i)
	{
		if(argv[i][0] == '-')
		{
			...
			else if(i+1 < argc)
			{
				   ...
				   strcmp(argv[i], "-s") == 0)
				{
					...
					{
						fprintf(stderr, "Error: calcif3: delayServerHost name, %s, is too long (more than %d chars)\n",
				...
				else if(strcmp(argv[i], "--server-type") == 0)
				{
					...
					{
						fprintf(stderr, "Error: calcif3: delayServerType type, %s, is not recognized\n", argv[i]);
						++die;
					}
				}
				else if(strcmp(argv[i], "--server-type") == 0)
				{
					++i;
					opts->delayServerHandlerType = stringToDelayServerHandlerType(argv[i]);
					if(opts->delayServerHandlerType == NumDelayServerHandlerTypes)
					{
						fprintf(stderr, "Error: calcif3: delayServerHandlerType type, %s, is not recognized\n", argv[i]);
						++die;
					}
					
				}
				else if(strcmp(argv[i], "--num-CALC-threads") == 0)
				{
					++i;
					opts->Num_CALC_Threads = uint_fast32_t(strtoul(argv[i], NULL, 0));
				}
				...
				{
					printf("Error: calcif3: Illegal option : %s\n", argv[i]);
					++die;
				}
			}
			else if(argv[i][0] == '-')
		...
		else
		{
			...
			{
				fprintf(stderr, "Error: calcif3: Too many files (%d max)\n", MAX_FILES);
				++die;
			}
		}
	}
	...
	{
		fprintf(stderr, "Error: calcif3: No input files!\n");
		++die;
	}

	if(((opts->polyOrder)) && (opts->polyOrder < 2 || opts->polyOrder > MAX_MODEL_ORDER))
	{
		fprintf(stderr, "Error: calcif3 Polynomial order must be in range [2, %d]\n", MAX_MODEL_ORDER);
		++die;
	}

	if(opts->polyOversamp < 1 || opts->polyOversamp > MAX_MODEL_OVERSAMP)
	{
		fprintf(stderr, "Error: calcif3 Polynomial oversample factor must be in range [1, %d]\n", MAX_MODEL_OVERSAMP);
		++die;
	}
	...
	{
		fprintf(stderr, "Error: calcif3: Interval must be in range [10, 600] sec\n");
		++die;
	}

	if(opts->nFile > 0 && opts->doall)
	{
		fprintf(stderr, "Error: calcif3: Option '--all' provided with files!\n");
		++die;
	}
	else if(opts->doall > 0)
	{
		...
		{
			fprintf(stderr, "Error: calcif3: No .calc files found.  Hint: Did you run vex2difx yet???\n");
			++die;
		}
		for(i = 0; i < opts->nFile; ++i)
		{
	...
	if(opts->delayServerHost[0] == 0)
	{
		...
		if(cs)
		{
			...
			{
				fprintf(stderr, "Error: calcif3: env var DIFX_DELAY_SERVER is set to a name that is too long, %s (should be < %d chars)\n", cs ? cs : "localhost", DIFXIO_HOSTNAME_LENGTH);
				++die;
			}
		}
	}
	...
	if(die)
	{
		if(die > 1)
		{
			fprintf(stderr, "calcif3 quitting. (%d errors)\n", die);
		}
		else
		{
			fprintf(stderr, "calcif3 quitting.\n");
		}
		fprintf(stderr, "Use -h option for calcif3 help.\n");
		deleteCommandLineOptions(opts);
...
static void tweakDelays(DifxInput *D, const char *tweakFile, int verbose)
{
	...
	if(nModified != nModel)
	{
		printf("WARNING: calcif3: Only %d of %d models modified!\n", nModified, nModel);
	}
	else
	{
		printf("calcif3: All %d models modified.\n", nModel);
	}
...
static int runfile(const char *prefix, const CommandLineOptions *opts, CalcParams *p)
{
	...
	{
		fprintf(stderr, "Error: calcif3: D->job->delta_lmn is zero, but D->job->perform_uvw_deriv is not None\n");
		return -1;
	}
	if((D->job->delta_lmn == 0.0) && ((D->job->perform_lmn_deriv)))
	{
		fprintf(stderr, "Error: calcif3: D->job->delta_lmn is zero, but D->job->perform_lmn_deriv is not None\n");
		return -1;
	}
	if((D->job->delta_xyz == 0.0) && ((D->job->perform_xyz_deriv)))
	{
		fprintf(stderr, "Error: calcif3: D->job->delta_xyz is zero, but D->job->perform_xyz_deriv is not None\n");
		return -1;
	}
	...
	if(difxVersion && D->job->difxVersion[0])
	{
		...
		{
			printf("Attempting to run calcif3 from version %s on a job make for version %s\n", difxVersion, D->job->difxVersion);
			...
			{
				fprintf(stderr, "calcif3 won't run on mismatched version without --override-version.\n");
				deleteDifxInput(D);
	...
	{
		printf("Warning: calcif3: working on unversioned job\n");
	}
	...
	if(opts->delayServerHandlerType != UnknownDelayServerHandler)
	{
		D->job->delayServerHandlerType = opts->delayServerHandlerType;
	}
	...
	{
		deleteDifxInput(D);
		fprintf(stderr, "Error: calcif3: difxCalcInit returned %d\n", v);
	...
	{
		deleteDifxInput(D);
		fprintf(stderr, "Error: calcif3: difxCalc returned %d\n", v);
	...
	tweakDelays(D, "calcif3.delay", opts->verbose);
	writeDifxIM(D);
	if(opts->verbose > 0)
	{
...
{
	if(p->delayDistributor)
	{
		delete p->delayDistributor; p->delayDistributor = NULL;
	}
	free(p->argument.station);
	free(p->argument.source);
	free(p->argument.EOP);
	free(p->response.result);
	free(p);
}
...
CalcParams *newCalcParams(const CommandLineOptions *opts)
{
	...
	p->Num_CALC_Threads = opts->Num_CALC_Threads;
	p->Num_Allocated_Stations = 0;
	p->Num_Allocated_Sources = 0;
	p->delayDistributor = NULL;
	p->argument_id = 0;
