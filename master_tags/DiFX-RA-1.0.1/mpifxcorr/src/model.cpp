...
bool Model::readScanData(istream * input)
{
	...
	for(int i=0;i<numscans;i++) {
		...
		config->getinputline(input, &(scantable[i].obsmodename), "SCAN ", i, " OBS MODE NAME");
		config->getinputline(input, &line, "SCAN ", i, " UVSHIFT INTERVAL (NS)");
		scantable[i].maxnsbetweenxcavg = atoi(line.c_str());
		config->getinputline(input, &line, "SCAN ", i, " AC AVG INTERVAL (NS)");
		...
		for(int j=0;j<scantable[i].numphasecentres;j++) {
			config->getinputline(input, &line, "SCAN ", i, " PHS CTR ", j);
...
bool Model::readEOPData(istream * input)
{
	...
	for(int i=0;i<numeops;i++) {
		config->getinputline(input, &line, "EOP ", i, " TIME (mjd)");
		eoptable[i].mjd = atoi(line.c_str());
		config->getinputline(input, &line, "EOP ", i, " TAI_UTC (sec)");
		eoptable[i].taiutc = atoi(line.c_str());
		config->getinputline(input, &line, "EOP ", i, " UT1_UTC (sec)");
		eoptable[i].ut1utc = atof(line.c_str());
		config->getinputline(input, &line, "EOP ", i, " XPOLE (arcsec)");
		eoptable[i].xpole = atof(line.c_str());
		config->getinputline(input, &line, "EOP ", i, " YPOLE (arcsec)");
...
bool Model::readPolynomialSamples(istream * calcinput)
{
	...
	for(int i=0;i<numstations;i++) {
		config->getinputline(input, &line, "TELESCOPE ", i, " NAME");
	...
	for(int i=0;i<numscans;i++) {
		...
		for(int j=0;j<scantable[i].numphasecentres;j++) {
			config->getinputline(input, &line, "SCAN ", i, " PHS CTR ", j, " SRC");
		...
		for(int j=0;j<scantable[i].nummodelsamples;j++) {
			...
			for(int k=0;k<scantable[i].numphasecentres+1;k++) {
				...
				for(int l=0;l<numstations;l++) {
					...
					config->getinputline(input, &line, "SRC ", k, " ANT ", l, " U (m)", true, 20);
