  ...
  : config(conf), configindex(confindex), datastreamindex(dsindex), recordedbandchannels(recordedbandchan), channelstoaverage(chanstoavg), blockspersend(bpersend), guardsamples(gsamples), fftchannels(recordedbandchan*2), numrecordedfreqs(nrecordedfreqs), numrecordedbands(nrecordedbands), numzoombands(nzoombands), numbits(nbits), unpacksamples(unpacksamp), fringerotationorder(fringerotorder), arraystridelength(arraystridelen), recordedbandwidth(recordedbw), blockclock(bclock), filterbank(fbank), linear2circular(linear2circular), calccrosspolautocorrs(cacorrs), recordedfreqclockoffsets(recordedfreqclkoffs), recordedfreqclockoffsetsdelta(recordedfreqclkoffsdelta), recordedfreqphaseoffset(recordedfreqphaseoffs), recordedfreqlooffsets(recordedfreqlooffs)
{
  ...
  else
  {
    ...
    // cverbose << startl << "Numstrides is " << numstrides << ", recordedbandchannels is " << recordedbandchannels << ", arraystridelength is " << arraystridelength << endl;
...
printf("lofreq was %f and is now %f\n", lofreq, lofreq-config->getDRecordedBandwidth(configindex, datastreamindex, i));
    ...
    // FIXME WFB: what to do about this block below?  Nothing may be the right answer.
    // WFB: In general, complex case needs to be carefully looked at for proper handling of LSB
    ...
    status = vectorMulC_f32(stepchannelfreqs, fracsampleerror - recordedfreqclockoffsets[i] + recordedfreqclockoffsetsdelta[i]/2, stepfracsamparg, numfracstrides/2);
    if(status != vecNoErr)
    ...
    // now do the first arraystridelength elements
	...
	exit(1);
      ...
      status = vectorMulC_f32(stepchannelfreqs, fracsampleerror - recordedfreqclockoffsets[i] - recordedfreqclockoffsetsdelta[i]/2, stepfracsamparg, numfracstrides/2);  //L2C add delay
      if(status != vecNoErr)
	csevere << startl << "Error in frac sample correction, arg generation (step)!!!" << status << endl;
	...
	// FIXME: WFB: Comparison of 'R' below should also include 'X' (or 'H')
...
// FIXME: WFB: Put in more accurate values here...
