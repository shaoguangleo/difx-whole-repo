...
/*******************************************************************************
 */
static void calc_9_1_raprog_1 (pRequest, pTransport)    /* server dispatch routine */
struct svc_req *pRequest;
SVCXPRT *pTransport;
/*
 * This function is called each time a client requests service.  It is passed 
 * an argument via the RPC specifing the requested CALC_9_1_RA data, i.e. the station 
...
{
	double ast_dopplr();
	getCALC_9_1_RA_arg argument;    /* RPC caller passes this */
	getCALC_9_1_RA_res result;      /* we return this to RPC caller */
	...
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 0\n");    
	tp = &timep;
	time (tp);
...
#warning "remove the following line when CALC_9_1_RA is fixed to support msa"
	...
	if (ilogdate != 0)
	{
		...
		if (itoday != ilogdate)
		{
			...
			snprintf (filename, 256, "CALC_9_RA_Server:%s%02d%02d%02d:%02d%02d%02d.log", hostname, 
			          timeptr->tm_year-100,
			...
			printf ("opened new CALC_9_1_RA_Server log file : %s\n", filename);
	...
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 1\n"); 
	/* handle null procedure */
	if (pRequest->rq_proc == NULLPROC)
	{
		fprintf(flog, "NULLPROC in calc_9_1_raprog_1 1\n"); 
	...
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 2\n"); 
	/* handle unimplemented procedure number */
	if (pRequest->rq_proc != GETCALC_9_1_RA)
	{
		fprintf(flog, "unimplemented procedure in calc_9_1_raprog_1 1\n"); 
		svcerr_noproc (pTransport);
		fflush(flog);
		return;
	}

	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 3\n"); 
	/* get RPC input argument */
	bzero (&argument, sizeof (argument));    /* see "The Art of DA" p193 */
	if (!svc_getargs (pTransport, xdr_getCALC_9_1_RA_arg, (char *)&argument))
	{
		fprintf(flog, "could not get RPC arguments in calc_9_1_raprog_1 1\n"); 
	...
	/* Signal CALC_9_1_RA through calcinit to write CALC_9_1_RA state stuff into
	...
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 4\n");
	...
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 4.1\n"); 

	delay *= 1.0e-6;

#if 1
	printf ("CALC_9_1_RAServer: delay = %20.14e\n", delay);
	printf ("CALC_9_1_RAServer: rate  = %e\n", rate);
	printf ("CALC_9_1_RAServer: dry, wet = %e %e\n", atmos[0], atmos[1]);
#endif

	time (tp);

	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 4.2\n");
	sock_in = svc_getcaller (pTransport);
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 4.3\n");
	fprintf(stdout, "JMA DEBUG: sock_in = %p\n", sock_in);
	fprintf(stdout, "JMA DEBUG: sock_in->sin_addr = 0x%8.8X\n", sock_in->sin_addr.s_addr);
	destdotaddr = inet_ntoa(sock_in->sin_addr);
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 5\n"); 
	...
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 6\n"); 
	...
	result.getCALC_9_1_RA_res_u.record.date  = argument.date;
	...
	result.getCALC_9_1_RA_res_u.record.wet_atmos[0] = atmos[1];
	result.getCALC_9_1_RA_res_u.record.wet_atmos[1] = atmos[1];
	result.getCALC_9_1_RA_res_u.record.dry_atmos[2] = datmos[0];
	result.getCALC_9_1_RA_res_u.record.dry_atmos[3] = datmos[0];
	result.getCALC_9_1_RA_res_u.record.wet_atmos[2] = datmos[1];
	result.getCALC_9_1_RA_res_u.record.wet_atmos[3] = datmos[1];

	result.getCALC_9_1_RA_res_u.record.el[0] = xelev[0];
	result.getCALC_9_1_RA_res_u.record.el[1] = xelev[1];
	result.getCALC_9_1_RA_res_u.record.az[0] = xaz[0];
	result.getCALC_9_1_RA_res_u.record.az[1] = xaz[1];
	result.getCALC_9_1_RA_res_u.record.msa[0]= xmsa[0];
	result.getCALC_9_1_RA_res_u.record.msa[1]= xmsa[1];
	result.getCALC_9_1_RA_res_u.record.el[2] = relev[0];
	result.getCALC_9_1_RA_res_u.record.el[3] = relev[1];
	result.getCALC_9_1_RA_res_u.record.az[2] = raz[0];
	result.getCALC_9_1_RA_res_u.record.az[3] = raz[1];
	result.getCALC_9_1_RA_res_u.record.msa[2] = rmsa[0];
	result.getCALC_9_1_RA_res_u.record.msa[3] = rmsa[1];
	result.getCALC_9_1_RA_res_u.record.UV[0] = u;
	result.getCALC_9_1_RA_res_u.record.UV[1] = v;
	result.getCALC_9_1_RA_res_u.record.UV[2] = w;
	result.getCALC_9_1_RA_res_u.record.riseset[0] = risetime;
	result.getCALC_9_1_RA_res_u.record.riseset[1] = settime;

	result.getCALC_9_1_RA_res_u.record.baselineP2000[0] = baselineJ2000[0];
	result.getCALC_9_1_RA_res_u.record.baselineP2000[1] = baselineJ2000[1];
	result.getCALC_9_1_RA_res_u.record.baselineP2000[2] = baselineJ2000[2];
	result.getCALC_9_1_RA_res_u.record.baselineV2000[0] = baselineJ2000[3];
	result.getCALC_9_1_RA_res_u.record.baselineV2000[1] = baselineJ2000[4];
	result.getCALC_9_1_RA_res_u.record.baselineV2000[2] = baselineJ2000[5];
	result.getCALC_9_1_RA_res_u.record.baselineA2000[0] = baselineJ2000[6];
	result.getCALC_9_1_RA_res_u.record.baselineA2000[1] = baselineJ2000[7];
	result.getCALC_9_1_RA_res_u.record.baselineA2000[2] = baselineJ2000[8];
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 7\n"); 
	...
	else
	{
		...
		result.getCALC_9_1_RA_res_u.record.struct_code = CALC_9_1_RA_SERVER_STRUCT_CODE_CURRENT;
	}

	/* return result to client */
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 8\n"); 

	if (!svc_sendreply (pTransport, xdr_getCALC_9_1_RA_res, (char *)&result))
		svcerr_systemerr (pTransport);
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 9\n"); 

	/* free any memory allocated by xdr routines when argument was decoded */
	if (!svc_freeargs (pTransport, xdr_getCALC_9_1_RA_arg, (char *)&argument))
	{
		fprintf (flog, "unable to free arguments %m\n");
		/* syslog (LOG_ERR, "unable to free arguments %m\n"); */
		exit(EXIT_FAILURE);
	}
	fprintf(stdout, "JMA DEBUG: inside calc_9_1_raprog_1 10\n"); 
	fflush(flog);
#if 1
	fflush(stdout);
#endif
 ...
 -*/
{
	...
	strcpy (Version, "CALC_9_1_RAServer Version 5.5");

	/* open system log file to send messages */
	/* openlog ("CALC_9_1_RA_Server", LOG_PID, LOG_USER); */
	...
	if (argc > 1)
	{
		...
		fprintf (stdout, "opened new CALC_9_1_RA_Server log file %s\n", filename);
		ilogdate = 0;
	}
	else
	{
	...
	(void) pmap_unset (CALC_9_1_RAPROG, CALC_9_1_RAVERS);

	/* create TCP transport handle */
	if ((pTransport = svctcp_create (RPC_ANYSOCK, 0, 0)) == NULL)
	{
		fprintf (flog, "cannot create tcp service\n");
		/* syslog (LOG_ERR, "cannot create tcp service %m\n"); */
		exit(EXIT_FAILURE);
	}

	/* register service with portmapper */
	if (svc_register (pTransport, CALC_9_1_RAPROG, CALC_9_1_RAVERS, 
	                  calc_9_1_raprog_1, IPPROTO_TCP) == 0)
	{
		fprintf(flog, "unable to register service\n");
	...
	fprintf (stderr, "svc_run returned %m\n");
