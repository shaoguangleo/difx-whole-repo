...
int pystream::maxIFs(const VexData *V) const
{
	...
	for(unsigned int modeNum = 0; modeNum < nMode; ++modeNum)
	{
		...
		if(setup->ifs.size() > maxIF)
		{
			maxIF = setup->ifs.size();
		}
	}
...
*/
		...
		if(personalityType == RDBE_PFB || personalityType == RDBE_DDC)
		{
			...
			*this << endl;
			// if we need a 2nd DBE, only DDC is supported
			if( need2DBEs ) {
				*this << "dbe1 = RDBE(1, 'ddc', " << dbeFilename << ")" << endl;
				*this << "dbe1.setALC(1)" << endl;
				*this << "dbe1.setFormat('Mark5B')" << endl;
				*this << "dbe1.setPSNMode(0)" << endl;
				*this << "dbe1.setPacket(0, 0, 40, 5008)" << endl;
...
/*			cerr << "IF2DBEassign for mode: " << modeNum << ": ";
			...
			cerr << "#### IF in use: " << IFinUse[modeNum] << " DBEs: " << numDBE << "(" << (need2DBEbyMode[modeNum]?2:1) << ") for mode:" << modeNum
				<< " IFinUseByDBE: " << IFinUseByDBE[modeNum][DBE_0] << ":" << IFinUseByDBE[modeNum][DBE_1] << endl;
*/
			string ifname;
			bool ifFound;
...
//				cerr << "# changing to mode " << mode->defName << endl;
				if(scriptType == SCRIPT_VLBA)
				{
...
/*            	    		        cerr << "found matching index: " << it3->first << " switch: "
						...
						*this << "subarray.setVLBALoIfSetup(loif" << modeId << dbeAppend << ")" << endl;
					}
				}
				else if(scriptType == SCRIPT_EVLA)
				{
